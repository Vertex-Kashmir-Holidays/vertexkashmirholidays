...
- **Wraps multi-step writes in a Prisma `$transaction`** — the established pattern for lead conversion, lead unlock, and itinerary updates; the online-payment path (verify-payment / webhook / reconcile) now shares this guarantee via `recordOnlinePayment`, which commits its ledger row and booking-status update atomically (VERTE-16). Connect chat sends follow the same rule: the message row, the sender’s read marker, and the recipients’ notifications commit in one transaction (VERTE-35).
