...
| shadcn/ui + Radix UI + class-variance-authority + tailwind-merge + clsx | Accessible unstyled primitives (Radix) + variant styling (CVA) + class merging (`cn()`) | 12 real primitives exist today — `Accordion`, `Sheet`, `Button`, `Dialog`, `Input`, `Label`, `Badge`, `Select`, `Avatar`, `Tooltip`, `Tabs`, `DropdownMenu` (`src/components/ui/`). `TooltipProvider` is wired once in both `(public)/layout.tsx` and `admin/layout.tsx` — any new `Tooltip` usage just works, no per-component provider needed. Most admin UI still hand-rolls markup rather than using these (tracked as a Design System backlog gap — `Popover`/`Checkbox`/`Switch`/`Combobox` are the next candidates). | New shared UI belongs in `src/components/ui/`, built on Radix + the `cn()` helper in `src/lib/utils.ts`. Don't hand-roll a modal/dropdown/tabs/etc. that Radix already solves.                              |
...
- `Tour`/`Campaign` JSON-string columns (`gallery`, `itinerary`, `highlights`, etc. — see `../instructions/coding-standards.md` → Database Standards → JSON Columns for the full 15/11-column list) must be `JSON.parse`'d on read with a fallback and `JSON.stringify`'d on write. Note `faqs` is a real `Faq[]` relation on both models, not one of these JSON-string columns.
...
| Upstash Redis + @upstash/ratelimit (`src/lib/ratelimit.ts`)                                            | Rate limiting                          | Booking creation/verification, every OTP request/verify route (register, forgot-password, careers), login, leads, newsletter, careers apply, admin MFA verify/confirm, `GET /api/spotlight`, and the password branch of `PATCH /api/account/profile` | Falls back to an in-memory limiter (per-serverless-instance, resets on redeploy) when Upstash env vars are unset — treat any rate-limit result as advisory defense-in-depth, never the sole guard on an endpoint. A rejected limit is always returned via the shared `tooManyRequests()` helper so every throttled route answers with the same `429` + `{ error }` + `Retry-After` shape — don't hand-roll a 429. |
