...
export default async function AboutPage() {
  ...
  const organizationPeopleJsonLd = buildOrganizationPeople({
    ...
    employees: team
      .filter((m) => m.id !== founder?.id)
      .map((m) => ({ name: m.name, jobTitle: m.role })),
  });
  // Short answers only — matches what FaqPreviewList actually renders below.
  const faqJsonLd =
    aboutFaqs.length > 0
      ? buildFAQPage(aboutFaqs.map((f) => ({ question: f.question, answer: f.shortAnswer })))
      : null;
  ...
  return (
    <div className="bg-background text-foreground">
      ...
      />
      <AboutPress
        label={content?.pressLabel ?? null}
        items={press.map((p) => sanitizePressHtml(p.html))}
      />
      {aboutFaqs.length > 0 && (
