...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return buildMetadata({
    title: "Contact Us — Plan Your Kashmir Trip",
    description:
...
export default async function ContactPage() {
  const [content, heroFeatures, promiseItems, faqs, offices, settings, testimonials] =
    await Promise.all([
      getContactContent(),
      prisma.contactHeroFeature.findMany({
        where: { isActive: true },
        orderBy: { sortOrder: "asc" },
      }),
      prisma.contactPromiseItem.findMany({
        where: { isActive: true },
        orderBy: { sortOrder: "asc" },
      }),
      ...
      <main className="mx-auto max-w-[1300px] px-6 py-12">
        <div className="grid items-start gap-8 lg:grid-cols-[1.05fr_300px]">
          <div className="min-w-0">
            ...
            <ContactPromise
              heading={{
                kicker: content?.promiseKicker ?? null,
                title: content?.promiseTitle ?? null,
              }}
          ...
          <section className="mt-16 border-t border-border pt-14">
            <div className="text-center">
              <p className="text-[12px] font-bold tracking-[0.22em] text-primary">
                {content?.faqsKicker ?? "QUESTIONS"}
              </p>
              <h2 className="h-display mt-2 font-display text-[18px] font-bold">
                {content?.faqsTitle ?? "Frequently Asked"}
              </h2>
            </div>
            <div className="mt-8">
            ...
            <div className="mt-6 flex justify-center">
              <Link
                href={
                  content?.faqsCtaHref && content.faqsCtaHref !== "#" ? content.faqsCtaHref : "/faq"
                }
