...
// Step 2 of the booking-form email verification: verify the emailed code.
// Does not touch any Booking record — it proves email ownership and issues a
// one-time verification token (returned once, stored only as a bcrypt hash)
// that create-order must be presented before it will create a booking.
export async function POST(req: NextRequest) {
  try {
    const ipLimit = await rateLimit(`otp-verify:booking:${clientIp(req)}`, 30, "10 m");
    if (!ipLimit.success) {
    ...
    // Scoped to purpose: BOOKING so a stray register/reset/careers OTP row for
    // the same email can never be verified here (and vice versa).
    const record = await prisma.emailOtp.findUnique({ where: { email } });
    if (!record || record.purpose !== "BOOKING") {
      return NextResponse.json(
    ...
    console.error("[bookings/verify-otp] error:", err);
    return NextResponse.json(
