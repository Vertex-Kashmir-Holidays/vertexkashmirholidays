...
import { isCloudinaryConfigured, ensureCloudinaryConfig, cloudinaryUploadFolder } from "@/lib/storage";
...
export async function POST(req: NextRequest) {
  ...
  if (!isCloudinaryConfigured()) {
    return NextResponse.json({ error: "Cloudinary is not configured on this server." }, { status: 503 });
  }
  ...
  const signature = cloudinary.utils.api_sign_request({ timestamp, folder, public_id: publicId }, apiSecret);
