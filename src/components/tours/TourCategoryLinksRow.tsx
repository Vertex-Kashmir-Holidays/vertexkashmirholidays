...
export function TourCategoryLinksRow({ categories }: { categories: TourCategory[] }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2.5">
      <span className="text-[12.5px] font-semibold text-muted-foreground">Browse by type:</span>
      {categories.map((c) => (
        <Link
          key={c}
          href={`/tours/category/${TOUR_CATEGORY_META[c].slug}`}
          className="rounded-full border border-border px-3.5 py-1.5 text-[12.5px] font-semibold transition hover:border-primary hover:bg-muted"
        >
