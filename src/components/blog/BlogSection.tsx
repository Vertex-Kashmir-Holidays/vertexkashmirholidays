...
export function BlogSection({ heading, blogs }: BlogSectionProps) {
  ...
  return (
    <section
      id="blogs"
      className="relative z-[2] mx-auto max-w-[1300px] px-4 pb-12 pt-16 sm:px-6 sm:pb-10 sm:pt-24"
    >
          ...
          >
            ...
            <div className="pop-sm p-5">
              <p className="text-[12px] text-muted-foreground">
                {[b.dateLabel, b.readTime ? `${b.readTime} min read` : null]
                  .filter(Boolean)
                  .join(" · ")}
              </p>
              <h3 className="mt-2 font-bold leading-snug text-foreground transition group-hover:text-primary">
                {b.title}
              </h3>
              {b.excerpt && (
