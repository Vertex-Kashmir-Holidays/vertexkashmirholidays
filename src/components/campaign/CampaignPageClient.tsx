...
export function CampaignPageClient({ campaign, footerSettings }: CampaignPageClientProps) {
  ...
  const whatsappHref =
    campaign.whatsappHref && campaign.whatsappHref !== "#"
      ...
      : wa(
          `Hi! I'm interested in the "${campaign.name}" Kashmir offer. Please share details, pricing and availability.`,
        );
  ...
  return (
    ...
    >
      ...
      />
      {campaign.tiers[0] && (
        <CampaignStickyMobileCTA
          price={campaign.tiers[0].price}
          cta={campaign.heroCta ?? "Reserve"}
        />
      )}
