...
const statusMap = {
  filling: {
    label: "FILLING FAST",
    className: "bg-amber-400/15 text-amber-500 dark:text-amber-300",
  },
  open: {
    label: "AVAILABLE",
    className: "bg-emerald-400/15 text-emerald-600 dark:text-emerald-300",
  },
  ...
  return (
    <section className="relative z-[2] mx-auto max-w-[900px] px-6 pt-20" id="departures">
        ...
        <div className="divide-y divide-border">
          {batches.map((batch, i) => {
            ...
            return (
              ...
              >
                <p className="col-span-2 text-[14px] font-bold text-foreground sm:col-span-1">
                  {batch.date}
                </p>
                <p
                  className={`text-[14px] font-semibold ${batch.seats > 0 && batch.seats <= 4 ? "text-amber-500 dark:text-amber-300" : "text-muted-foreground"}`}
                >
                ...
                <div className="flex items-center justify-between gap-3 sm:justify-end">
                  <span
                    className={`rounded-full px-3 py-1 text-[12px] font-extrabold ${status.className}`}
                  >
                    {status.label}
                  </span>
                  {!isSold && (
      ...
      >
        ...
        <a href="#reserve" className="font-bold text-camp-accent hover:underline">
          Talk to us
        </a>{" "}
        — we run custom departures for groups of 4+.
      </motion.p>
    </section>
  );
}
