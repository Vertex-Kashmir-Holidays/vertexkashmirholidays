...
const topAttractionItemSchema = z.object({
  name: z.string().default(""),
  description: z.string().default(""),
});
const foodOrShopItemSchema = z.object({
  name: z.string().default(""),
  description: z.string().default(""),
});
...
export function DestinationForm({ defaults, activityOptions = [], blogOptions = [] }: Props) {
  ...
  const {
    ...
    defaultValues: {
      ...
      topAttractions: (defaults?.topAttractions ?? []).map((a) => ({
        name: a.name,
      ...
      localFood: (defaults?.localFood ?? []).map((f) => ({
        name: f.name,
        description: f.description,
      })),
      shopping: (defaults?.shopping ?? []).map((s) => ({
        name: s.name,
  ...
  const {
    fields: whyVisitFields,
    append: addWhyVisit,
    remove: removeWhyVisit,
  } = useFieldArray({ control, name: "whyVisit" });
  const {
    fields: attractionFields,
    append: addAttraction,
    remove: removeAttraction,
  } = useFieldArray({ control, name: "topAttractions" });
  const {
    fields: foodFields,
    append: addFood,
    remove: removeFood,
  } = useFieldArray({ control, name: "localFood" });
  const {
    fields: shopFields,
    append: addShop,
    remove: removeShop,
  } = useFieldArray({ control, name: "shopping" });
  const {
    fields: tipFields,
    append: addTip,
    remove: removeTip,
  ...
  function onSubmit(data: FormData) {
    ...
    const payload = {
      ...
      topAttractions: stringifyList(
        topAttractions.filter((a) => a.name.trim() || a.description.trim()),
      ),
    ...
    startTransition(async () => {
      try {
        ...
        if (!res.ok) {
          if (res.status === 403) {
            toast.error(
              "You don't have permission to save destinations. Contact your administrator.",
            );
            return;
          }
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6 space-y-4">
            <div>
              ...
              <p className="text-[12px] text-muted-foreground mt-0.5">
                Shown on destination cards &amp; detail page. Coordinates power the live weather
                widget.
              </p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              >
                ...
                <input
                  ...
                  disabled={uploading}
                  onChange={(e) =>
                    e.target.files?.[0] && uploadFile(e.target.files[0], "coverImage")
                  }
                />
              </label>
            </div>
            ...
            <div className="space-y-2">
              {whyVisitFields.map((field, i) => (
                <div key={field.id} className="flex gap-2 items-center">
                  ...
                  <input
                    {...register(`whyVisit.${i}.value`)}
                    className={`${inputCls} flex-1`}
                    placeholder="e.g. Alpine meadows blooming with wildflowers in spring"
                  />
                  <button
                    type="button"
              ...
              {whyVisitFields.length === 0 && (
                <p className="text-xs text-muted-foreground py-2">No reasons added yet.</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => addAttraction({ name: "", description: "" })}
                className="flex items-center gap-1 text-xs font-semibold text-primary hover:text-primary/80 transition-colors"
              >
                ...
                {attractionFields.map((field, i) => (
                  ...
                  >
                    <div className="flex-1 space-y-2">
                      <input
                        {...register(`topAttractions.${i}.name`)}
                        className={inputCls}
                        placeholder="Attraction name — e.g. Betaab Valley"
                      />
                      <textarea
                        {...register(`topAttractions.${i}.description`)}
                        rows={2}
                        className={`${inputCls} resize-none`}
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6 space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-muted-foreground mb-1">
                Best Time to Visit — Detail
              </label>
              <p className="text-[12px] text-muted-foreground mb-1">
                HTML is accepted. Full seasonal breakdown — the short Best Season field above stays
                for the sidebar quick-fact.
              </p>
              <textarea
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className={`${inputCls} resize-none`}
                placeholder="Air / road / rail directions..."
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-muted-foreground mb-1">
              ...
              <p className="text-[12px] text-muted-foreground mb-1">HTML is accepted.</p>
              <textarea
                {...register("whereToStay")}
                rows={4}
                className={`${inputCls} resize-none`}
                placeholder="Hotel / stay category guidance..."
              />
            </div>
          </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => addFood({ name: "", description: "" })}
                className="flex items-center gap-1 text-xs font-semibold text-primary hover:text-primary/80 transition-colors"
              >
              ...
              <div className="space-y-3">
                {foodFields.map((field, i) => (
                  ...
                  >
                    <div className="flex-1 space-y-2">
                      <input
                        {...register(`localFood.${i}.name`)}
                        className={inputCls}
                        placeholder="Dish / place — e.g. Wazwan"
                      />
                      <textarea
                        {...register(`localFood.${i}.description`)}
                        rows={2}
                        className={`${inputCls} resize-none`}
                        placeholder="Short description..."
                      />
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => addShop({ name: "", description: "" })}
                className="flex items-center gap-1 text-xs font-semibold text-primary hover:text-primary/80 transition-colors"
              >
                <Plus className="w-3.5 h-3.5" /> Add Item
              </button>
              ...
              <div className="space-y-3">
                {shopFields.map((field, i) => (
                  ...
                  >
                    <div className="flex-1 space-y-2">
                      <input
                        {...register(`shopping.${i}.name`)}
                        className={inputCls}
                        placeholder="Item / market — e.g. Pashmina shawls"
                      />
                      <textarea
                        {...register(`shopping.${i}.description`)}
                        rows={2}
                        className={`${inputCls} resize-none`}
                        placeholder="Short description..."
                      />
            ...
            <div className="space-y-2">
              {tipFields.map((field, i) => (
                <div key={field.id} className="flex gap-2 items-center">
                  ...
                  <input
                    {...register(`travelTips.${i}.value`)}
                    className={`${inputCls} flex-1`}
                    placeholder="e.g. Carry a postpaid SIM — prepaid connections don't work in J&K"
                  />
                  <button
                    type="button"
                    onClick={() => removeTip(i)}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6 space-y-4">
            <div>
              ...
              <p className="text-[12px] text-muted-foreground mt-0.5">
                Activities shown on this destination&apos;s page. Manage activities in the
                Activities module.
              </p>
            </div>
            <LinkChecklist
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6 space-y-4">
            <div>
              ...
              <p className="text-[12px] text-muted-foreground mt-0.5">
                Curated editorial links shown at the end of this destination&apos;s page. Not an
                automatic feed — manage blog posts in the Blogs module.
              </p>
            </div>
            <LinkChecklist
              title="Blog Posts"
              options={blogOptions}
              value={relatedBlogIds}
              onChange={setRelatedBlogIds}
            />
          </div>
