  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-display font-extrabold text-brand-navy text-xl">Destinations</h2>
          <p className="text-gray-400 text-xs mt-0.5">Manage Kashmir destinations and their content</p>
        </div>
        <Link
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.length === 0 ? (
                ...
                filtered.map((dest) => (
                  <tr key={dest.id} className={cn("hover:bg-gray-50/50 transition-colors", confirmDelete === dest.id && "bg-red-50/30")}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3 max-w-xs">
                        ...
                        <div className="min-w-0">
                          <p className="font-semibold text-brand-navy text-xs leading-tight truncate max-w-[160px]">{dest.name}</p>
                          <p className="text-[10px] text-gray-400 truncate">/destinations/{dest.slug}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-xs font-semibold text-brand-navy">
                      {dest._count?.tours ?? 0}
                    </td>
