  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-5">
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="text-xs font-semibold text-foreground" htmlFor="ap-name">Full name</label>
              <input id="ap-name" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} placeholder="Your name" />
            </div>
            <div>
              <label className="text-xs font-semibold text-foreground" htmlFor="ap-email">Email</label>
              <input id="ap-email" value={email} disabled className={`${inputCls} cursor-not-allowed bg-muted text-muted-foreground`} />
            </div>
          </div>
          <div>
            <label className="text-xs font-semibold text-foreground">Role</label>
            <p className="mt-1.5 inline-flex rounded-lg bg-muted px-2.5 py-1 text-xs font-bold text-muted-foreground">{role}</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <p className="text-xs text-muted-foreground">Enter your current password to set a new one. Leave blank to keep it.</p>
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          <div>
            <label className="text-xs font-semibold text-foreground" htmlFor="ap-current">Current password</label>
            <input id="ap-current" type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} className={inputCls} autoComplete="current-password" />
          </div>
          <div>
            <label className="text-xs font-semibold text-foreground" htmlFor="ap-new">New password</label>
            <input id="ap-new" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className={inputCls} autoComplete="new-password" />
          </div>
          <div>
            <label className="text-xs font-semibold text-foreground" htmlFor="ap-confirm">Confirm new password</label>
            <input id="ap-confirm" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className={inputCls} autoComplete="new-password" />
          </div>
        </div>
      </div>
      ...
      <GalleryPicker open={pickerOpen} type="IMAGE" onSelect={setImage} onClose={() => setPickerOpen(false)} />
    </form>
  );
}
