...
interface ContactOfficeMapProps {
  ...
  // Additional corporate offices beyond the primary map-anchored one above
  // (e.g. a 2nd branch) — rendered as a simple list further down. Already
  // excludes the primary office (the page splits [primary, ...others] once
  // from the same ContactOffice query before passing this down).
...
export function ContactOfficeMap({ content, offices: otherOffices }: ContactOfficeMapProps) {
  const hasRegisteredLegalInfo = content.legalName || content.tourismRegNumber || content.gstNumber;

  return (
    <>
      {/* Three sections: Corporate Office (30%) | Map (40%) | Registered Office (30%). */}
      <section className="mt-14 grid items-start gap-7 lg:grid-cols-[3fr_4fr_3fr]">
        ...
        >
          ...
          {content.subtitle && (
            <p className="mt-2 text-[14px] text-muted-foreground">{content.subtitle}</p>
          )}
          ...
          <ul className="mt-6 space-y-4 text-[14px]">
            ...
            {content.phone && (
              <li className="flex items-center gap-2.5">
                ...
                <a href={`tel:${content.phone.replace(/\s/g, "")}`} className="hover:text-primary">
                  {content.phone}
                </a>
              </li>
            )}
        ...
        >
          {mapEmbedKey && content.placeId ? (
            ...
            <svg
              ...
              role="img"
              aria-label="Map showing the Vertex Kashmir Holidays head office"
            >
              ...
              <path
                d="M250 90 C330 70 420 95 440 150 C460 205 430 250 380 265 C320 282 260 270 230 230 C205 196 200 130 250 90 Z"
                fill="hsl(200 65% 80%)"
              />
              <path
                d="M300 130 C330 122 360 135 365 158 C370 180 355 198 330 202 C305 206 285 196 280 175 C276 158 282 138 300 130 Z"
                fill="hsl(200 60% 86%)"
              />
              ...
              <g fontFamily="Inter, sans-serif" fill="hsl(206 12% 45%)" fontWeight="600">
                <text x="330" y="185" fontSize="15" letterSpacing="2" fill="hsl(205 45% 45%)">
                  DAL LAKE
                </text>
                <text x="560" y="65" fontSize="11" letterSpacing="1.5">
                  NISHAT BAGH
                </text>
                <text x="470" y="160" fontSize="11" letterSpacing="1.5">
                  HAZRATBAL
                </text>
                <text x="430" y="335" fontSize="11" letterSpacing="1.5">
                  LAL CHOWK
                </text>
                <text x="610" y="320" fontSize="11" letterSpacing="1.5">
                  BUNIAT
                </text>
                <text x="60" y="170" fontSize="11" letterSpacing="1.5">
                  DARGAH
                </text>
                <text x="230" y="385" fontSize="11" letterSpacing="1.5">
                  RAJBAGH
                </text>
                <text x="380" y="378" fontSize="11" letterSpacing="1.5">
                  RABAGH
                </text>
              </g>
              <g transform="translate(318 218)">
                ...
                <path
                  d="M0 -14 C-11 -14 -18 -6 -18 4 C-18 16 0 34 0 34 C0 34 18 16 18 4 C18 -6 11 -14 0 -14 Z"
                  fill="hsl(355 80% 55%)"
                />
          ...
          {!mapEmbedKey || !content.placeId
            ? content.mapLabel && (
                <div className="absolute left-[26%] top-[18%] rounded-xl bg-card px-4 py-3 shadow-card">
                  ...
                  {content.mapSubLabel && (
                    <p className="text-[12px] text-muted-foreground">{content.mapSubLabel}</p>
                  )}
        ...
        {/* Registered Office — legal/permanent address, informational only
            (no map or directions). Tourism Reg. No. / GSTIN live here since
            they're legal-identity details, not corporate-office details. */}
        {content.registeredOffice && (
          ...
          >
            ...
            <h2 className="h-display mt-2 font-display text-[18px] font-bold">
              {content.registeredOffice.name}
            </h2>

            <ul className="mt-6 space-y-4 text-[14px]">
              <li className="flex items-start gap-2.5">
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-primary" strokeWidth={2} />
                <span className="text-muted-foreground">{content.registeredOffice.address}</span>
              </li>
            </ul>

            {hasRegisteredLegalInfo && (
              <p className="mt-5 border-t border-border pt-5 text-[12px] leading-relaxed text-muted-foreground">
                ...
                {content.gstNumber && <br />}
                {content.gstNumber && `GSTIN: ${content.gstNumber}`}
              </p>
            )}
          </motion.div>
        )}
      ...
      {/* Additional corporate offices (beyond the primary one above) —
          appears automatically as more are added via /admin/contact. */}
      {otherOffices.length > 0 && (
        <div className="mt-8 border-t border-border pt-6">
          <p className="text-sm font-bold text-foreground">Other Offices</p>
          <ul className="mt-3 grid gap-4 text-[14px] sm:grid-cols-2 lg:grid-cols-3">
            {otherOffices.map((o) => (
