...
export function DestinationDetailOverview({
  name,
  description,
  features = [],
}: DestinationDetailOverviewProps) {
  return (
    <motion.section
      id="overview"
    ...
    >
      ...
      <p className="mt-3 max-w-2xl text-[14px] leading-relaxed text-foreground/75">{description}</p>
      {features.length > 0 && (
        <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
          {features.map((feature, i) => (
            ...
            >
              <span
                className={`mx-auto grid h-12 w-12 place-items-center rounded-xl ${feature.color}`}
              >
                <svg
                  viewBox="0 0 24 24"
                  className="h-6 w-6"
