// src/components/destinations/DestinationWhereToStay.tsx
...
export function DestinationWhereToStay({ html }: DestinationWhereToStayProps) {
  if (!html) return null;
  const clean = sanitizeHtml(html, SANITIZE_OPTIONS);
  if (!clean.trim()) return null;

  return (
    <section id="where-to-stay" className="rounded-2xl border border-border bg-card p-3 sm:p-6 shadow-soft">
      <h2 className="text-[17px] font-bold">Where to Stay</h2>
      <div
