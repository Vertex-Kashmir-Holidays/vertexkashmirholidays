          ...
          <div className="rv relative h-[420px]" style={{ '--rd': '0.18s' } as any} data-depth-group>
            ...
            {content.cardTitle && (
              <div className="glass-strong absolute right-4 top-2 rounded-2xl px-5 py-4 shadow-card" data-depth style={{ '--d': '1.6' } as any}>
                ...
                <p className="mt-1 text-xs font-bold text-white">{content.cardTitle}</p>
                {content.cardSubtitle && <p className="text-[10px] text-white/55">{content.cardSubtitle}</p>}
              </div>
            )}
            {content.ratingTitle && (
              <div className="glass-strong absolute -bottom-3 left-6 rounded-2xl px-5 py-4 shadow-card" data-depth style={{ '--d': '1.3' } as any}>
                <p className="text-xs font-bold text-white">{content.ratingTitle}</p>
                {content.ratingSubtitle && <p className="text-[10px] text-white/55">{content.ratingSubtitle}</p>}
              </div>
            )}
          </div>
        </div>
